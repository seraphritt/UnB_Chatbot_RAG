...
for model in models:
    ...
    print(result)
    df = result.to_pandas()
    csv_file_name = f"evaluation_results_GRAPH_UNB{model_name}.csv"
    ...
    with PdfPages(f'boxplot_graph_UNB{model_name}.pdf') as pdf:
        pdf.savefig()
        plt.close()
    print(f"Documento evaluation_results_UNB{model_name}.csv e boxplot (boxplot_graph_UNB{model_name}_teste.pdf) criados")
